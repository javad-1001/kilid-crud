...
export class MapService {
  ...
  createMarker(position: any, icon: any, id = '', name = '', data = {}) {
  ...
  drawRouteAnimate(
   ...
   // origCoordinates:any,
    ...
    var lastCoordinate = coordinates[coordinates.length - 1];
    var endMarker = this.craeteArrow(
      ...
      {
        html:`<span class="text-success">زمان : ${lastCoordinate.strTime}</span>
           <br />
           <span class="text-danger">سرعت : ${lastCoordinate.iSpeed}</span>
           `,
      },
      lastCoordinate.iAngle
    );
    map.getView().setCenter(olProj.fromLonLat([lastCoordinate.fLon, lastCoordinate.fLat]))
    var points = [];
    var pointFeatures = [];
    for (const point of coordinates) {
      let coord4326 = olProj.transform(
  ...
  craeteArrow(position: any, icon: any, id = '', name = '', data = {},Rotation:any) {
    ...
    fitMap(data: any, map: any) {
      ...
      var source = [
        Math.min(transformedStart[0], transformedEnd[0]) - 6000,
        Math.min(transformedStart[1], transformedEnd[1]) - 6000,
        Math.max(transformedStart[0], transformedEnd[0]) + 6000,
        Math.max(transformedStart[1], transformedEnd[1]) + 6000,
      ];
      view.fit(source);
      if (data.length == 1) {
